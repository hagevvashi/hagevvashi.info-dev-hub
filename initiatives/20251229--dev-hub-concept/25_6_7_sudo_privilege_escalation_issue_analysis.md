...
3. **25_6_6の誤った仮説**: "docker-entrypoint.sh は非rootユーザー（hagevvashi）として実行される" という前提が誤り
...
1. **USER ディレクティブの欠如**:
   - Dockerfileに `USER ${UNAME}` ディレクティブが存在しない
...
3. **25_6_6での誤認**:
   - セクション14.3で「docker-entrypoint.sh は非rootユーザー（hagevvashi）として実行される」と記載
...
s6-setuidgid hagevvashi /usr/local/bin/docker-entrypoint.sh
```
...
**誤認の訂正**:
以前の分析では「docker-entrypoint.sh は非rootユーザー（hagevvashi）として実行される」と記載していたが、これは誤りであった。
